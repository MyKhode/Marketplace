...
export default {
  setup() {
    ...
    // Watch for route changes (if route params change)
    watch(() => route.params.id, () => {
      fetchProducts(); // Fetch new products whenever the route changes
    });

    // Watch for active category changes
    watch(activeCategory, () => {
      fetchProducts(); // Refetch products when the category changes
    });
  ...
  <Suspense>
    <template #default>
      <div id="app"
        ...
        <div class="tags mt-5 md:mt-0 col-span-12 md:col-span-6 md:justify-self-end">
        ...
        <h2 class="col-span-12 text-indigo-600 text-xl font-bold my-12" v-if="!loading && filteredProducts.length === 0">
          No products found!
        </h2>
        <div class="col-span-12 grid grid-cols-12 gap-5 mt-12">
          <!-- Skeleton Loader -->
          <div v-if="loading" class="col-span-12 md:col-span-6 lg:col-span-4 sm:flex bg-white rounded-lg p-5 gap-5 h-full">
            <SkeletonProduct />
            <SkeletonProduct />
            <SkeletonProduct />
          </div>
          ...
          <div v-for="(product, index) in filteredProducts" :key="index"
            ...
            <router-link :to="`/product/${product.id}`" class="w-full">
              <img :src="product.image" :alt="product.name" class="rounded-lg w-full h-48 object-cover" />
